...
const KitchenDashboard: React.FC = () => {
  ...
  const [newEntries, setNewEntries] = useState<{ orders: Set<string>, items: Set<string> }>({
    orders: new Set(),
    items: new Set()
  });
  ...
  useEffect(() => {
    // Only detect if there was a previous state (not on fresh load)
    if (prevOrdersRef.current.length > 0) {
      const addedOrderIds: string[] = [];
      const addedItemIds: string[] = [];

      orders.forEach(order => {
        const prevOrder = prevOrdersRef.current.find(po => po.order_id === order.order_id);

        // Check for new order
        if (!prevOrder) {
          addedOrderIds.push(order.order_id);
          // All items in a new order are also "new"
          order.items_ordered.forEach(item => addedItemIds.push(`${order.order_id}-${item.id}`));
        } else {
          // Check for new items in existing order
          order.items_ordered.forEach(item => {
            const isNewItem = !prevOrder.items_ordered.some(pi => pi.id === item.id);
            if (isNewItem) {
              addedItemIds.push(`${order.order_id}-${item.id}`);
            }
          });
        }
      });

      if (addedOrderIds.length > 0 || addedItemIds.length > 0) {
        setNewEntries(prev => {
          const nextOrders = new Set(prev.orders);
          const nextItems = new Set(prev.items);
          addedOrderIds.forEach(id => nextOrders.add(id));
          addedItemIds.forEach(id => nextItems.add(id));
          return { orders: nextOrders, items: nextItems };
        });

        // Auto-clear "new" status after 60 seconds
        setTimeout(() => {
          setNewEntries(prev => {
            const nextOrders = new Set(prev.orders);
            const nextItems = new Set(prev.items);
            addedOrderIds.forEach(id => nextOrders.delete(id));
            addedItemIds.forEach(id => nextItems.delete(id));
            return { orders: nextOrders, items: nextItems };
          });
        }, 60000);
      }
        ...
        {sortedOrders.length === 0 ? (
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {sortedOrders.map((order) => {
              const isNewOrder = newEntries.orders.has(order.order_id);
              return (
                <div
                  key={order.order_id}
                  className={`bg-gray-50 rounded-lg shadow-md p-5 border border-gray-200 flex flex-col transition-all duration-500 ${isNewOrder ? 'animate-pulse-gold' : ''}`}
                >
                  <div className="flex justify-between items-center mb-4 pb-3 border-b border-gray-200 gap-2">
                    <div className="flex items-center">
                      <h3 className="text-lg md:text-xl font-bold text-gray-900">{import.meta.env.VITE_TABLE_LABEL || 'Table'}: {order.table_number_or_online}</h3>
                      {isNewOrder && <span className="badge-new ml-2">New Order</span>}
                    </div>
                  ...
                  <ul className="mb-4 flex-grow space-y-2">
                    {order.items_ordered.map((item) => {
                      const isNewItem = newEntries.items.has(`${order.order_id}-${item.id}`);
                      return (
